...
export default function StudentDetailPage() {
  ...
  const resetPassword = async () => {
    setBusy(true);
    try {
      const response = await fetch(`${API_BASE}/admin/students/${studentId}/reset-password`, { method: 'POST', credentials: 'include' });
      const body = await response.json().catch(() => ({}));
      if (!response.ok || body.success === false) throw new Error(body.error || 'Could not reset password.');
      setResetResult({ username: body.login_username, password: body.temporary_password });
      setToast({ message: 'Password reset', isError: false });
    } catch (err) { setToast({ message: err.message, isError: true }); } finally { setBusy(false); }
  };
