...
# ---------- Day 41 task: public students API ----------

@app.route("/api/students", methods=["GET"])
def api_students():
    """
    Public endpoint for the Day 41 frontend-backend integration task.
    ...
    The `students` table has no email column, so a placeholder email is
    derived from each student's roll_no.
    """
    conn = get_connection()
    if not conn:
        return jsonify({"success": False, "error": "DB connection failed"}), 500
    cursor = conn.cursor(dictionary=True)
    cursor.execute("SELECT id, name, roll_no, course FROM students ORDER BY id")
    ...
    students = [
        {
            "id": row["id"],
            "name": row["name"],
            "email": f"{row['roll_no'].lower()}@crescent.edu",
            "course": row["course"],
        }
        for row in rows
    ]
