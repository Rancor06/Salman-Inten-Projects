...
export default function StudentIntelligence() {
  ...
  const [students, setStudents] = useState([]); const [loading, setLoading] = useState(true); const [error, setError] = useState(''); const [search, setSearch] = useState(''); const [adding, setAdding] = useState(false); const [editing, setEditing] = useState(null); const [toast, setToast] = useState({ message: '', isError: false });
  ...
  useEffect(() => { load(); }, []);
  const filtered = useMemo(() => { const query = search.trim().toLowerCase(); return query ? students.filter((student) => [student.name, student.roll_no, student.course].some((value) => String(value || '').toLowerCase().includes(query))) : students; }, [students, search]);
